...
class ImplementationGen
{
    ...
    static std::string typeToString( Operations::ReturnTypes::ReturnType* pReturnType )
    {
        ...
        else if( auto pDimension = db_cast< Operations::ReturnTypes::Dimension >( pReturnType ) )
        {
            ...
            VERIFY_RTE_MSG(
                canonicalTypes.size() == 1, "Cannot generate runtime return type for non-homogeneous dimension" );
            os << *canonicalTypes.begin();
        }
        else if( auto pFunctionReturnType = db_cast< Operations::ReturnTypes::Function >( pReturnType ) )
        {
            ...
            VERIFY_RTE_MSG(
                canonicalTypes.size() == 1, "Cannot generate runtime return type for non-homogeneous function" );
            VERIFY_RTE_MSG(
            ...
            os << strReturnType << "(*)( " << mega::psz_mega_reference << "* ";
...
public:
    ...
    void generate( Operations::Invocations* pInvocations, std::ostream& os )
    {
        ...
        for( auto& [ id, pInvocation ] : pInvocations->get_invocations() )
        {
            ...
            std::string strParameterType;
            {
                if( auto pWrite = db_cast< Operations::Write >( pInvocation ) )
                {
                    strParameterType = typeToString( pWrite->get_parameter_type() );
                }
            }

            const std::string strRuntimeReturnType = typeToString( pInvocation->get_return_type() );

            nlohmann::json invocation( {

                { "return_type", pInvocation->get_return_type()->get_canonical_type() },
                { "runtime_return_type", strRuntimeReturnType },

                { "parameter_type", strParameterType },
