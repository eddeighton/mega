...
public:
    ...
    constexpr inline explicit SubObjectID( c_concrete_sub_object_id value )
        : c_concrete_sub_object_id( value )
    {
    }
...
{
    return os << "0x" << std::hex << std::setw( 4 ) << std::setfill( '0' ) << value.getValue() << std::dec << "_CSO";
}
