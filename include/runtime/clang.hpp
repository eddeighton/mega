...
{
public:
    Clang( const MegastructureInstallation& megaInstall );

    void compileToLLVMIR( const boost::filesystem::path& cppFile, const boost::filesystem::path& irFile,
                          std::optional< const FinalStage::Components::Component* > pComponent ) const;

private:
    const MegastructureInstallation& m_megaInstall;
};
