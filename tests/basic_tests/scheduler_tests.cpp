...
public:
    struct ExecutionState
    {
        ...
        mega::runtime::Pointer       m_ref;
        mega::ActionCoroutine m_coroutine;
        EventVector           m_events;

        ExecutionState( ExecutionStateImpl& impl, const mega::runtime::Pointer& ref )
            : m_impl( impl )
            , m_ref( ref )
        {
        }

        const mega::runtime::Pointer& getRef() const { return m_ref; }
        ...
        void onEvent( const mega::runtime::Pointer& ev ) { m_events.push_back( ev ); }
    ...
    {
        mega::runtime::Pointer ref( mega::TypeInstance::make_object( mega::TypeID::make_object_from_objectID( objectTypeID ) ),
...
struct TestActions
{
    ...
    mega::ActionCoroutine start( const mega::runtime::Pointer& ref )
    {
...
{
    mega::runtime::Pointer    ref2, ref3;
...
TEST_F( BasicSchedulerSleep, CallAndStop )
{
    ...
    mega::runtime::Pointer ref1 = ex1.getRef();
    mega::runtime::Pointer ref2 = ex2.getRef();
    mega::runtime::Pointer ref3 = ex3.getRef();
    scheduler.call( std::move( ex1 ) );
    scheduler.call( std::move( ex2 ) );
    scheduler.call( std::move( ex3 ) );
    ASSERT_EQ( scheduler.size(), 3U );
