...
using ConcreteFixtureType = CompilerTestFixture< Data >;
} // namespace

TEST_P( ConcreteFixtureType, ConcreteParameterizedTest )
{
    const Data data = GetParam();

    try
    {
        auto pCompilation = createBuildAndRun( data.sourceFiles, data.includeFiles, data.testName.c_str(), "Task_ConcreteTree" );

        mega::pipeline::PipelineResult result = pCompilation->runPipeline( "" );
        ASSERT_TRUE( result.m_bSuccess );

        pCompilation->generateReport( "/?report=Concrete", data.testName );
    }
