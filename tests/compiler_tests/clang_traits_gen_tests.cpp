...
using ClangTraitsGenFixtureType = CompilerTestFixture< Data >;
} // namespace

TEST_P( ClangTraitsGenFixtureType, ClangTraitsGenParameterizedTest )
{
    const Data data = GetParam();

    try
    {
        auto pCompilation = createBuildAndRun( data.sourceFiles, data.includeFiles, data.testName.c_str(), "Task_Clang_Traits_Gen" );

        mega::pipeline::PipelineResult result = pCompilation->runPipeline( "" );
        ASSERT_TRUE( result.m_bSuccess );

        pCompilation->generateReport( "/?report=ClangTraitsGen", data.testName );
    }
