...
boost::filesystem::path                g_compiler_templatesDir;
boost::filesystem::path                g_report_templatesDir;
boost::filesystem::path                g_stashDir;
boost::filesystem::path                g_inputDir;
boost::filesystem::path                g_expectedDir;
...
int inner_main( int argc, char* argv[] )
{
    ...
    boost::filesystem::path input, expected, result;
    ...
    desc.add_options()
        ...
        ( "input",          bpo::value< boost::filesystem::path >( &input ),            "Input test source file directory" )
        ( "expected",       bpo::value< boost::filesystem::path >( &expected ),         "Expected test source file directory" )
        ( "result",         bpo::value< boost::filesystem::path >( &result ),           "Result test source file directory" )
    ...
    {
        // clang-format off
        VERIFY_RTE_MSG( boost::filesystem::exists( input ), "File not found input at : " << input.string() );
        VERIFY_RTE_MSG( boost::filesystem::exists( expected ), "File not found expected at : " << expected.string() );
        VERIFY_RTE_MSG( boost::filesystem::exists( result ), "File not found result at : " << result.string() );
