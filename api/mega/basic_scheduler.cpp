...
namespace
{
    ...
    public:
        void allocated( const mega::reference& ref, mega::Scheduler::StopperFunctionPtr pStopper)
        {
        ...
        void stopperStopped( const mega::reference& ref )
        {
        ...
        void signal( const mega::reference& ref )
        {
            on_event( m_events_by_ref_wait, ref );
            on_event( m_events_by_ref_sleep, ref );
        }
    
        void call( const mega::reference& ref, mega::Scheduler::StopperFunctionPtr pStopper, mega::Scheduler::ActionOperator actionOperator )
        {
            ActiveAction* pAction = 
                new ActiveAction
                ( 
        ...
        void stop( const mega::reference& ref )
        {
            ActiveActionMap::iterator iFind = m_actions.find( ref );
            if( iFind != m_actions.end() )
            {
        ...
        void pause( const mega::reference& ref )
        {
            ActiveActionMap::iterator iFind = m_actions.find( ref );
            if( iFind != m_actions.end() )
            {
        ...
        void unpause( const mega::reference& ref )
        {
            ActiveActionMap::iterator iFind = m_actions.find( ref );
            if( iFind != m_actions.end() )
            {
        ...
        void cycle()
        {
            ...
            while( !( getActive().empty() && getWaiting().empty() ) )
            {
                ...
                if( m_pCurrentAction )
                {
                    const mega::ReturnReason& reason = 
