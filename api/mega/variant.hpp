...
struct [[clang::eg_type( mega::id_Variant )]] __eg_variant
{
    inline __eg_variant()
    {
        data.address.value = mega::INVALID_ADDRESS;
    }
    
    inline __eg_variant( const Event& from )
    {
        //when convert from event need to check runtime type against all __eg_variant types
        if( ( from.data.address.value == mega::INVALID_ADDRESS ) || !mega::variant_runtime_type_check< Ts... >::test( from ) )
        {
            data.address.value = mega::INVALID_ADDRESS;
        }
        else 
    ...
    {
        if( data.address.value != mega::INVALID_ADDRESS )
        {
