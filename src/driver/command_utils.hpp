...
pathListToFiles( const std::vector< std::string >& pathList )
{
    ...
    for ( const std::string& strPath : pathList )
    {
        ...
        VERIFY_RTE_MSG( boost::filesystem::exists( sourceFilePath ),
                        common::COLOUR_RED_BEGIN << "ERROR: Failed to locate: "
                                                 << sourceFilePath.string() << common::COLOUR_END );
...
inline std::vector< boost::filesystem::path >
pathListToFiles( const boost::filesystem::path&    sourceDirectory,
...
inline std::vector< boost::filesystem::path >
pathListToFolders( const std::vector< std::string >& pathList )
{
    std::vector< boost::filesystem::path > result;
    for ( const std::string& strPath : pathList )
    {
        ...
        VERIFY_RTE_MSG( boost::filesystem::exists( folderPath ),
                        common::COLOUR_RED_BEGIN << "ERROR: Failed to locate: "
                                                 << folderPath.string() << common::COLOUR_END );
