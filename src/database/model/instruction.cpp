...
void ASTSerialiser::load( io::Loader& loader, const ASTElement*& pElement )
{
    bool bNull = false;
    loader.load( bNull );
    if ( bNull )
    ...
    else
    {
        ...
        else
        {
            ...
            case eInstanceVariable:
                pNewElement = new InstanceVariable;
                break;
            case eReferenceVariable:
                pNewElement = new ReferenceVariable;
                break;
            case eDimensionVariable:
                pNewElement = new DimensionReferenceVariable;
                break;
            case eContextVariable:
                pNewElement = new ContextVariable;
                break;

            case eRootInstruction:
                THROW_RTE( "Unreachable" );
            case eParentDerivationInstruction:
                pNewElement = new ParentDerivationInstruction;
                break;
            case eChildDerivationInstruction:
                pNewElement = new ChildDerivationInstruction;
                break;
            case eEnumDerivationInstruction:
                pNewElement = new EnumDerivationInstruction;
                break;
            case eEnumerationInstruction:
                pNewElement = new EnumerationInstruction;
                break;
            ...
            case eDimensionReferenceReadInstruction:
                pNewElement = new DimensionReferenceReadInstruction;
                break;
            case eMonoReferenceInstruction:
                pNewElement = new MonomorphicReferenceInstruction;
                break;
            case ePolyReferenceInstruction:
                pNewElement = new PolymorphicReferenceBranchInstruction;
                break;
            case ePolyCaseInstruction:
                pNewElement = new PolymorphicCaseInstruction;
                break;

            case eCallOperation:
                pNewElement = new CallOperation;
                break;
            case eStartOperation:
                pNewElement = new StartOperation;
                break;
            case eStopOperation:
                pNewElement = new StopOperation;
                break;
            case ePauseOperation:
                pNewElement = new PauseOperation;
                break;
            case eResumeOperation:
                pNewElement = new ResumeOperation;
                break;
            case eDoneOperation:
                pNewElement = new DoneOperation;
                break;
            case eWaitActionOperation:
                pNewElement = new WaitActionOperation;
                break;
            case eWaitDimensionOperation:
                pNewElement = new WaitDimensionOperation;
                break;
            case eGetActionOperation:
                pNewElement = new GetActionOperation;
                break;
            case eGetDimensionOperation:
                pNewElement = new GetDimensionOperation;
                break;
            case eReadOperation:
                pNewElement = new ReadOperation;
                break;
            case eWriteOperation:
                pNewElement = new WriteOperation;
                break;
            case eWriteLinkOperation:
                pNewElement = new WriteLinkOperation;
                break;
            case eRangeOperation:
                pNewElement = new RangeOperation;
                break;
            default:
                break;
            }
...
void ASTSerialiser::store( io::Storer& storer, const ASTElement* pElement )
{
    if ( nullptr == pElement )
    {
        storer.store( true );
...
///////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////
void Variable::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    serialiser.load( loader, m_pParent );
}
void Variable::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    serialiser.store( storer, m_pParent );
}

void InstanceVariable::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Variable::load( serialiser, loader );
    m_pType = loader.loadObjectRef< concrete::Action >();
}
void InstanceVariable::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Variable::store( serialiser, storer );
    storer.storeObjectRef( m_pType );
}

void ReferenceVariable::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Variable::load( serialiser, loader );
    loader.loadObjectVector( m_types );
}
void ReferenceVariable::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Variable::store( serialiser, storer );
    storer.storeObjectVector( m_types );
}

void DimensionReferenceVariable::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Variable::load( serialiser, loader );
}
void DimensionReferenceVariable::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Variable::store( serialiser, storer );
}

void ContextVariable::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Variable::load( serialiser, loader );
}
void ContextVariable::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Variable::store( serialiser, storer );
}
...
///////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////
void Instruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
...
void Instruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
...
Instruction::Elimination Instruction::eliminate()
{
    ...
    for ( std::size_t sz = 0U; sz != m_children.size(); ++sz )
    {
        ...
        if ( EliminationInstruction* pElimination = dynamic_cast< EliminationInstruction* >( pChild ) )
        {
            ...
            for ( Instruction* pNested : nested )
            {
                ...
                {
                case eSuccess:
                    success.push_back( pNested );
                    break;
                case eFailure:
                    failure.push_back( pNested );
                    break;
                case eAmbiguous:
                    ambiguous.push_back( pNested );
                    break;
                }
            }
        ...
        else if ( FailureInstruction* pFailure = dynamic_cast< FailureInstruction* >( pChild ) )
        {
            if ( elimination != eAmbiguous )
                elimination = eFailure;
        }
        else if ( PruneInstruction* pPrune = dynamic_cast< PruneInstruction* >( pChild ) )
        {
...
Instruction::Elimination Instruction::secondStageElimination( const std::vector< const Operation* >& candidateOperations )
{
    ...
    {
        std::vector< const Operation* >::const_iterator iFind = std::find( candidateOperations.begin(), candidateOperations.end(), pOperation );
...
void RootInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pContext );
    loader.load( m_iMaxRanges );
}
void RootInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
...
void ParentDerivationInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pFrom );
    serialiser.load( loader, m_pTo );
}
void ParentDerivationInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
...
void ParentDerivationInstruction::evaluate( RuntimeEvaluator& evaluator ) const
{
    ...
    const reference to{ from.instance / m_pFrom->getConcreteType()->getLocalDomainSize(),
                        static_cast< mega::TypeID >( m_pTo->getConcreteType()->getIndex() ), from.timestamp };
...
void ChildDerivationInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pFrom );
    serialiser.load( loader, m_pTo );
}
void ChildDerivationInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pFrom );
    serialiser.store( storer, m_pTo );
...
void ChildDerivationInstruction::evaluate( RuntimeEvaluator& evaluator ) const
{
    ...
    const reference to{ from.instance, static_cast< mega::TypeID >( m_pTo->getConcreteType()->getIndex() ), from.timestamp };
...
void EnumDerivationInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pFrom );
    serialiser.load( loader, m_pTo );
}
void EnumDerivationInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pFrom );
    serialiser.store( storer, m_pTo );
...
void EnumerationInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pContext );
...
void EnumerationInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pContext );
...
void EnumerationInstruction::evaluate( RuntimeEvaluator& evaluator ) const
{
    ...
    if ( !operations.empty() )
    {
        ...
        {
        case RangeOperation::eRaw:
            rangeDescription.raw = true;
            break;
        case RangeOperation::eRange:
            rangeDescription.raw = false;
            break;
        default:
            THROW_RTE( "Unknown range type" );
        }
    }
...
void FailureInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    THROW_RTE( "Unreachable" );
}
void FailureInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    THROW_RTE( "Unreachable" );
}

void EliminationInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    THROW_RTE( "Unreachable" );
}
void EliminationInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    THROW_RTE( "Unreachable" );
}

void PruneInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    THROW_RTE( "Unreachable" );
}
void PruneInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    THROW_RTE( "Unreachable" );
}

void DimensionReferenceReadInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
...
void DimensionReferenceReadInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
...
void MonomorphicReferenceInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pFrom );
    serialiser.load( loader, m_pInstance );
}
void MonomorphicReferenceInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pFrom );
    serialiser.store( storer, m_pInstance );
...
void PolymorphicReferenceBranchInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pFrom );
}
void PolymorphicReferenceBranchInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pFrom );
}
void PolymorphicReferenceBranchInstruction::evaluate( RuntimeEvaluator& evaluator ) const
{
    ...
    for ( const Instruction* pChild : m_children )
    {
        const PolymorphicCaseInstruction* pCase = dynamic_cast< const PolymorphicCaseInstruction* >( pChild );
        ASSERT( pCase );
...
void PolymorphicCaseInstruction::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
    serialiser.load( loader, m_pReference );
    serialiser.load( loader, m_pTo );
}
void PolymorphicCaseInstruction::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
    serialiser.store( storer, m_pReference );
    serialiser.store( storer, m_pTo );
...
///////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////
void Operation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Instruction::load( serialiser, loader );
}
void Operation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Instruction::store( serialiser, storer );
}

void CallOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
...
void CallOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
...
void StartOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void StartOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void StopOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void StopOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void PauseOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void PauseOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void ResumeOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void ResumeOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void DoneOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void DoneOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void WaitActionOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void WaitActionOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void WaitDimensionOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
...
void WaitDimensionOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void GetActionOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void GetActionOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void GetDimensionOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Dimension >();
...
void GetDimensionOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void ReadOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Dimension >();
...
void ReadOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void WriteOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Dimension >();
...
void WriteOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
void WriteLinkOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Dimension >();
...
void WriteLinkOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////
void RangeOperation::load( ASTSerialiser& serialiser, io::Loader& loader )
{
    Operation::load( serialiser, loader );
    serialiser.load( loader, m_pInstance );
    m_pInterface = loader.loadObjectRef< interface::Context >();
...
void RangeOperation::store( ASTSerialiser& serialiser, io::Storer& storer ) const
{
    Operation::store( serialiser, storer );
    serialiser.store( storer, m_pInstance );
    storer.storeObjectRef( m_pInterface );
...
ExplicitOperationID RangeOperation::getExplicitOperationType() const
{
    ...
    case eRange:
        return id_exp_Range;
    default:
    case TOTAL_RANGE_TYPES:
        THROW_RTE( "Invalid range type" );
    }
}
