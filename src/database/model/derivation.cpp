...
void DerivationAnalysis::load( io::Loader& loader )
{
    {
        std::size_t szSize = 0;
        loader.load( szSize );
...
void DerivationAnalysis::store( io::Storer& storer ) const
{
    {
...

    {
        ...
        for ( InheritanceNodeMap::const_iterator
                  i
              = m_inheritanceMap.begin(),
              iEnd = m_inheritanceMap.end();
              i != iEnd;
              ++i )
        {
            storer.storeObjectRef( i->first );
