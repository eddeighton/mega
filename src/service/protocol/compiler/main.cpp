        ...
        else
        {
            ...
            if ( bGenerateJSON )
            {
                ...
                for ( const Path& sourceFilePath : inputSourceFiles )
                {
                    ...
                    protocol::schema::Schema      schema;
                    protocol::schema::ParseResult result
                        = protocol::schema::parse( strFileContents, schema, osError );
                    if ( result.bSuccess )
                    {
                        ...
                        if ( iterReached == strFileContents.end() )
                        {
                            ...
                            protocol::jsonconv::toJSON( dataFolderPath, sourceFilePath.filename().replace_extension( "" ).string(), schema );
                        }
                        else
                        {
                            const int distance
