...
namespace
{
    ...
    boost::filesystem::path generateUndoFilePath( const std::string& szUniqueID, std::size_t szIndex )
    {
        std::ostringstream os;
        os <<   "undo_" << szUniqueID << 
                '_' << boost::posix_time::to_simple_string(
                            boost::posix_time::second_clock::universal_time() ) << 
                '_' << szIndex;

        boost::filesystem::path newPath = 
            style_replace_non_alpha_numeric( os.str(), '_' );
...
namespace editor
{
    ...
    boost::filesystem::path Document::UndoHistory::getUndoFileName() const
    {
        ...
        {
            newPath = 
                generateUndoFilePath( m_document.getUniqueObjectName(), szUniqueID );
            ++szUniqueID;
            bIsUnique = true;
    ...
    //////////////////////////////////////////////////
    //////////////////////////////////////////////////
    Document::Document( task::Scheduler& scheduler )
    ...
    Document::Document( task::Scheduler& scheduler, const boost::filesystem::path& path )
        :   m_scheduler( scheduler ),
            m_optPath( path ),
            m_uniqueObjectName( generateUUID() ),
            m_undoHistory( *this )
    ...
    void Document::reschedule()
    {
        ...
        if( map::File::Ptr pFile = getFile() )
        {
            m_pSchedule = pFile->createSchedule( m_compilationConfig );
            if( m_pSchedule )
            {
                m_pRun = m_scheduler.run( this, m_pSchedule );
            }
        }
    }
    ...
    void Document::onEditted( bool bCommandCompleted )
    {
        if( map::File::Ptr pFile = getFile() )
        {
            if( pFile->getLastModifiedTickForTree() > 
                    m_undoHistory.getLastRecordTick() )
            {
    ...
    //////////////////////////////////////////////////
    //////////////////////////////////////////////////
    SchematicDocument::SchematicDocument( task::Scheduler& scheduler, map::Schematic::Ptr pSchematic )
        :   Document( scheduler ),
    ...
    SchematicDocument::SchematicDocument( task::Scheduler& scheduler, 
                map::Schematic::Ptr pSchematic, 
                const boost::filesystem::path& path )
        :   Document( scheduler, path ),
            m_pSchematic( pSchematic )
    {
        m_undoHistory.onNewVersion();
    }
    ...
    void SchematicDocument::undo()
    {
        ...
        map::Schematic::Ptr pUndoSchematic = 
            boost::dynamic_pointer_cast< map::Schematic >( pUndoFile );
