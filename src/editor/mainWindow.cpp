...
:   QMainWindow( pParent ),
    ...
    m_scheduler( m_fifo ),
    m_bQuit( false ),
    m_fifoMonitorThread( std::bind( &MainWindow::fifoMonitor, this ) )
{
	m_pMainWindowImpl->setupUi( this );
    
    {
        //delete undo history from prior execution
        const boost::filesystem::path tempFolder( 
            boost::filesystem::current_path() / "history" );
    ...
    
    {
        ...
        //toolbox
        QObject::connect( 
            m_pMainWindowImpl->toolBox, SIGNAL( currentChanged(int) ), 
            m_pMainWindowImpl->toolBox, SLOT( onCurrentPaletteChanged(int) ) );
    ...
    QObject::connect(
        m_pMainWindowImpl->actionExit, SIGNAL(triggered()),
        this, SLOT( close() ) );
    
    QObject::connect(
        m_pMainWindowImpl->actionSave_All, SIGNAL(triggered()),
        this, SLOT( OnSaveAll() ) );
    ...
    */
    ...
    QObject::connect( 
        m_pDockManager, &ads::CDockManager::floatingWidgetCreated, 
        this,           &MainWindow::OnFloatingWidgetCreated );
    
    QObject::connect( 
        m_pDockManager, &ads::CDockManager::focusedDockWidgetChanged, 
        this,           &MainWindow::OnFocusedDockWidgetChanged );


    QObject::connect(
        this, &MainWindow::OnDocumentTaskCompleted,
        this, &MainWindow::OnDocumentTaskCompletedSlot, Qt::QueuedConnection );
...
{
    m_bQuit = true;
    m_fifoMonitorThread.join();
    
	delete m_pMainWindowImpl;
}

void MainWindow::fifoMonitor()
{
    while( !m_bQuit )
    {
        ...
        while( !m_fifo.empty() )
        {
            const task::Status status = m_fifo.pop();
            ...
            switch( status.m_state )
            {
                ...
                case task::Status::eSucceeded :
                    {
                        emit OnDocumentTaskCompleted( status.m_owner );
                    }
                    break;
...
bool MainWindow::TestClosingDocument( DocViewWidget& docView )
{
    ...
    if( docView.pDocument->isModified() )
    {
        ...
        {
            os << "Document: " << docView.pDocument->getFilePath().value().string() << 
                " has unsaved changes.";
        }
        else
        {
        ...
        const int Result = QMessageBox::question( this, tr( "Schematic Editor" ),
...
void MainWindow::closeEvent( QCloseEvent* pEvent )
{
    ...
    for( DocumentViewMap::iterator 
            i = m_docViewMap.begin(),
            iEnd = m_docViewMap.end(); i!=iEnd; ++i )
    {
...
void MainWindow::OnDocumentTaskCompletedSlot( const void* pDocument )
{
    for( DocumentViewMap::iterator 
            i = m_docViewMap.begin(),
            iEnd = m_docViewMap.end(); i!=iEnd; ++i )
    {
        ...
        {
            if( SchematicView* pSchematicView = 
                dynamic_cast< SchematicView* >( docView.pView ) )
            {
...
void MainWindow::OnNewSchematic( SchematicDocument::Ptr pNewSchematic )
{
    ...
    QObject::connect( 
        pDockWidget,    &ads::CDockWidget::closeRequested, 
        [ pThis, pDockWidget ](){ pThis->OnDockWidgetCloseRequested( pDockWidget ); } );
    
    QObject::connect( 
        pDockWidget,    &ads::CDockWidget::closed, 
        [ pThis, pDockWidget ](){ pThis->OnDockWidgetClosed( pDockWidget ); } );

    QObject::connect(
        pNewView, &SchematicView::OnDocumentSaved,
        this, &MainWindow::OnDocumentSaved );
        
    QObject::connect( 
        pNewView, &SchematicView::OnClipboardAction, 
        m_pMainWindowImpl->toolBox, &MissionToolbox::updateToolbox );
...
void MainWindow::OnNewBase()
{
    ...
    SchematicDocument::Ptr pNewSchematic( new SchematicDocument( m_scheduler, pNewBase ) );
...
void MainWindow::OnNewShip()
{
    ...
    SchematicDocument::Ptr pNewSchematic( new SchematicDocument( m_scheduler, pNewShip ) );
    OnNewSchematic( pNewSchematic );
}
...
void MainWindow::OnLoadSchematic( ClipboardMsg msg )
{
    ...
    {
        SchematicDocument::Ptr pNewSchematic( new SchematicDocument( m_scheduler, msg.pSchematic, msg.optFilePath.value() ) );
        OnNewSchematic( pNewSchematic );
    }
    else
    {
        SchematicDocument::Ptr pNewSchematic( new SchematicDocument( m_scheduler, msg.pSchematic ) );
        OnNewSchematic( pNewSchematic );
    }
}
...
void MainWindow::OnLoad()
{
    ...
    if( !strFilePath.empty() )
    {
        try
        {
            ...
            {
                SchematicDocument::Ptr pNewSchematic( new SchematicDocument( m_scheduler, pSchematic, filePath ) );
                OnNewSchematic( pNewSchematic );
            }
            else if( map::Mission::Ptr pMission = 
                boost::dynamic_pointer_cast< map::Mission >( pFile ) )
            {
                THROW_RTE( "TODO" );
            }
            else 
        ...
        catch( std::exception& ex )
        {
            ...
            QMessageBox::warning( this, tr( "Edittor" ),
                                  QString::fromUtf8( os.str().c_str() ) );
        }
    }
}
